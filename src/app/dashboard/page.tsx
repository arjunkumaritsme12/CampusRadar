  ...
  return (
    <div className="flex flex-col gap-8">
      <div>
        ...
        <p className="text-secondary-foreground/70">Here's what's happening with your placements.</p>
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-heading font-bold">Nearest Upcoming Drives</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {upcomingDrives.slice(0, 3).map((drive) => (
            <TiltCard key={drive.id}>
              <Link href={`/dashboard/drive/${drive.id}`} className="block h-full glass p-5 rounded-2xl border border-border/50 hover:shadow-lg hover:border-primary/50 transition-colors">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <h3 className="font-bold text-lg">{drive.company_name}</h3>
                    <p className="text-sm text-secondary-foreground/70">{drive.role}</p>
                  </div>
                ...
                <div className="space-y-2 text-sm mt-6">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{drive.scheduled_date ? new Date(drive.scheduled_date).toLocaleDateString() : 'TBD'}</span>
                  </div>
                  <div className="flex justify-between">
          ...
          {upcomingDrives.length === 0 && (
            <div className="col-span-full py-12 text-center border border-dashed border-border rounded-2xl text-secondary-foreground/50">
...
function StatCard({ title, value, icon, bg }: { title: string, value: string, icon: React.ReactNode, bg: string }) {
  return (
    <div className="glass p-6 rounded-2xl flex items-center gap-4 border border-border/50 hover:-translate-y-1 transition-transform">
      <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${bg}`}>
        {icon}
      </div>
      <div>
        <p className="text-sm font-medium text-secondary-foreground/60">{title}</p>
